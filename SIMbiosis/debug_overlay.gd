...
@export var moon_light_node_path : int
...
func _ready():
	...
	if not show:
		# Unfreeze the time.
		GlobalTime.freeze_time = false
		queue_free()

	# Connect signals.
	var current_minute_changed_signal = GlobalTime.connect(
		current_minute_changed, self, _on_minute_changed
	)

	var current_hour_changed_signal = Time.connect("current_hour_changed", Callable(self, "_on_hour_changed"))
	...
	freeze_time_checkbox.button_pressed = Time.freeze_time
	...
	if moon_light_node_path:
		...
		if (
			...
			show_moon_light_checkbox.button_pressed = true
		else:
